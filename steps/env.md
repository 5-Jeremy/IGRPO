...
| | |
|---|---|
| Conda env | `igrpo` @ `~/.conda/envs/igrpo` — Python 3.12, 12 GB |
| Retriever env | `retriever` @ `~/.conda/envs/retriever` — Python 3.10, pre-existing |
| Repo | `/scratch/user/debajoym98_tamu.edu/IGRPO` |
| Data, models, checkpoints | `/scratch/project/prj-02-llm-reasoning-shakkottai/debajoy/IGRPO` |
| Cluster | partition `def`, account `prj-02-llm-reasoning-shakkottai`, 8×H200 per node |
...
```bash
cd /scratch/user/debajoym98_tamu.edu/IGRPO
...
`df -h /scratch` reports petabytes, which is misleading: `/scratch/user/debajoym98_tamu.edu`
has a **1 TB quota**, and was at 908 GB used / 117 GB free. The wiki-18 assets alone need
81 GB steady and ~150 GB peak. Everything heavy therefore lives in the project directory
(2.5 TB free). Only the repo stays on `/scratch/user`.
...
```bash
source /sw/eb/sw/Miniforge3/25.3.0-3/etc/profile.d/conda.sh
...
`DATA=/scratch/project/prj-02-llm-reasoning-shakkottai/debajoy/IGRPO` — 81 GB.
...
```bash
export DATA=/scratch/project/prj-02-llm-reasoning-shakkottai/debajoy/IGRPO
