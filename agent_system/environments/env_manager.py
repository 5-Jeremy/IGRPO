...
class SearchEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Tuple[Dict[str, Any], List[Dict]]:
        ...
        self.memory.reset(batch_size=len(obs), tasks=obs)
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success, **kwargs):
        tree_structure = kwargs.get('tree_structure', False)
        if tree_structure:
            for i in range(len(total_batch_list)):
                batch_item = total_batch_list[i]
                if batch_item['is_terminal']:
                    info = total_infos[i]
